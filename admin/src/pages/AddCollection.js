...
const AddCollection = () => {
    const { handleNewCollection, getAllCollection, collections } = useContext(Context)
    ...
    const handleAddCollection = async () => {
        await handleNewCollection(image, updatesData)
        await getAllCollection()
    }
    return (
        <div>
            <PageTitle>Add New Collection</PageTitle>
            ...
            <div className="flex text-gray-800 dark:text-gray-300">
                ...
                {">"}
                <p className="mx-2">Add new Collection</p>
            </div>
            ...
            <div className="w-full mt-8 grid gap-4 grid-col md:grid-cols-3 ">
                <Card className="row-span-2 md:col-span-2">
                    <CardBody>
                        ...
                        <div className="w-full" onClick={() => handleAddCollection()}>
                                ...
                                <div>
                                    <ul>
                                        {collections?.map((brand) => (
                                            <li key={brand.id} className="my-2">{brand?.attributes?.collectionName}</li>
                                        ))}
                                    </ul>
                                </div>
                            )}
